...
// Define a custom error type that extends ValidationErrors
export interface $error extends ValidationErrors {
  message?: string;
  index?: number;
}

// Define a type for an array of errors or null
export type $errors = ($error | null)[];

// Define a rule type for file validation
export type $rule = {
  size: number;
  types: FileType[];
  required?: boolean;
...
// Define a base data transfer object
...
// Define a FileData class that extends BaseDto
...
// Define an enum for supported file types
...
// Define a type for file categories
export type FileCat = 'image' | 'document' | 'excel' | 'archive';

// Define file categories and their associated file types
...
// Define a type for file library information
export type FileLib = {
  hex: string[];
  mime: string;
  extensions: string;
...
// Define a library of file information for each file type
export const FilesLib: { [key in FileType]: FileLib } = {
  ...
  [FileType.RARarchive]: {
    hex: ['52 61 72 21 1A 07 00', '52 61 72 21 1A 07 01 00'],
    mime: 'application/vnd.rar, application/octet-stream',
    extensions: `.${FileType.RARarchive}`,
    offset: [0, 8]
  },
  [FileType.Text]: {
    hex: ['EF BB BF', 'FF FE', 'FE FF', 'FF FE 00 00', '00 00 FE FF', '0E FE FF'],
    mime: 'text/plain',
...
// Define a file utility class
export class FileUtil {
  // Read and validate files based on the specified options
  static async from(files: FileList, options?: $rule): Promise<FileData[]> {
    return await Promise.all(
      Array.from(files).slice(0, options?.chunk || 1).map(file => this.read(file, options))
    );
  }

  // Read and validate a single file based on the specified options
  ...
  // Validate files based on the specified options
      ...
      for (let i = 0; i < files?.length; ++i) {
        ...
        if (files[i]?.name) {
          ...
          if (typeCheck && !extensions.includes(`.${extension}`)) {
            errors[i] = {
              message: `The file should be one of ${options.types.join(' or ')}`,
              value: extension,
              index: i,
            };
            continue;
  ...
  // Evaluate the size in a human-readable format
